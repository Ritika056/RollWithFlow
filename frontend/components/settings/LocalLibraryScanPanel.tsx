...
export function LocalLibraryScanPanel() {
  ...
  return (
    <GlassCard className="music-sheen p-5 md:p-6">
      <div className="flex items-start gap-3"><div className="grid size-11 shrink-0 place-items-center rounded-2xl border border-signal/25 bg-signal/10 text-signal"><FolderSearch size={20} /></div><div><p className="text-xs font-semibold uppercase tracking-[0.2em] text-signal">Advanced local library</p><h2 className="mt-1 text-xl font-semibold text-white">Scan a backend-accessible folder</h2><p className="mt-2 text-sm leading-6 text-white/58">For normal use, go to Library and choose Upload Audio or Import Folder. Use this only when the backend can already access the folder path.</p></div></div>
