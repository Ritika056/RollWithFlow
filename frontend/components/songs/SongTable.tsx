...
import { Edit3, FolderPlus, Heart, RotateCcw, Trash2 } from "lucide-react";
      ...
      <div className="hidden overflow-x-auto soft-scrollbar xl:block">
        <table className="w-full min-w-[1120px] text-left text-sm">
          ...
          <tbody className="divide-y divide-white/8">
            {songs.map((song, index) => <SongRow key={song.id} song={song} index={index} onLikeToggle={onLikeToggle} onEdit={onEdit} onAddToFolder={onAddToFolder} onReject={onReject} onRemoveFromFolder={onRemoveFromFolder} />)}
          </tbody>
        </table>
      </div>
      <div className="grid gap-3 p-3 xl:hidden">
        {songs.map((song, index) => <SongCard key={song.id} song={song} index={index} onLikeToggle={onLikeToggle} onEdit={onEdit} onAddToFolder={onAddToFolder} onReject={onReject} onRemoveFromFolder={onRemoveFromFolder} />)}
      </div>
    </div>
  );
}
